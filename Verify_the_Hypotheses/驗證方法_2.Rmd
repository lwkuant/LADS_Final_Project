...
###方法: 先將每個詞彙、成句每個假設下受試者的分數平均起來，作為其分數，再透過兩個存活與否的判定標準(每個月平均出現頻率大於等於一次、變異係數小於等於一)把詞彙和成句分成存活和死亡組，最後再去利用two-sample t test去看每個假設下存活組和死亡組的平均分數有無顯著差異，來確立假設的分數不同和存活與否是否有關係的結論。
